...
public class ServeisPortalRestControllerTest extends RestServerParentTest {
	...
	/**
	 * Test stage 26 get descarregar document signat expedient.
	 *
	 * @throws Exception
	 *             the exception
	 */
	@Test
	public void testStage26_GetDescarregarDocumentSignatExpedient() throws Exception {

		when(clientEntity.getUsuariInteressat()).thenReturn("79688341B");

		String url = BASE_URL + "/expedients/1/documents/1/signat";
		getMockMvc().perform(get(url)).andDo(print()).andExpect(status().isOk());
