...
public class RespostaConsultaExpedientsMockService {
	...
	public RespostaConsultaExpedientsRDTO getRespostaConsultaExpedients(BigDecimal idExpedient) {
		...
		expedient.setProcediment(
				ProcedimentsRDTOFactory.create(
						new BigDecimal("1"), "PROC-01", "Procediment 1", 
						UnitatsRDTOFactory.create("UG1", "Unitat Gestora 1"), 
						Arrays.asList(
								UnitatsRDTOFactory.create("UG1", "Unitat Gestora 1"),
								UnitatsRDTOFactory.create("UG2", "Unitat Gestora 2")), 
						"OR-1", 
						Arrays.asList(new String[]{"OFICI","ADMINISTRACIO"}), "1", "EN_ELABORACIO")
				);
		
		expedient.setRegistre(
				RegistreRDTOFactory.create("0801930008-1-2018-0002094-1", "01/10/2018 14:00:02", 
					PersonesRDTOFactory.create(null, "FISICA", "Nom", "Cognom 1", "Cognom 2", 
						DocumentIdentitatRDTOFactory.create("NIF", "00000000T", "ES")))
		);
		
		
		expedient.setSolicitant(
				PersonesRDTOFactory.create(null, "FISICA", "Nom", "Cognom 1", "Cognom 2", 
						DocumentIdentitatRDTOFactory.create("NIF", "00000000T", "ES"))		
		);
		
		expedient.setRepresentant(
				PersonesRDTOFactory.create(null, "FISICA", "Nom", "Cognom 1", "Cognom 2", 
						DocumentIdentitatRDTOFactory.create("NIF", "00000000T", "ES"))		
		);		
		...
		expedient.setDocuments(
				Arrays.asList(
						DocumentsExpedientsRDTOFactory.create(new BigDecimal("1"), "SOLC", "FORM_DOC_SOLC.pdf"),
						DocumentsExpedientsRDTOFactory.create(new BigDecimal("2"), "NIF", "FORM_DOC_NIF.pdf")
				)
		);	
		
		expedient.setAtributs(
				Arrays.asList(
						DadesAtributsExpedientsRDTOFactory.create(new BigDecimal("1"), "ATTR1", "Valor Atribut 1"),
	...
	private static class HistoricsRDTOFactory {
		...
		private static HistoricsRDTO create(
				String estat,
				String dataCreacio,
			    String descripcio
		) {
	...
	private static class ProcedimentsRDTOFactory {
		...
		private static ProcedimentsConsultaRDTO create(
				BigDecimal id, 
				String codi, 
				String descripcio,
	...
	private static class PersonesRDTOFactory {
		...
		private static PersonesRDTO create(
				BigDecimal id,
				String tipusPersona,
			    String nomRaoSocial,
			    String cognom1,
	...
	private static class DocumentIdentitatRDTOFactory {
		...
		private static DocumentsIdentitatRDTO create(
	...
	private static class RegistreRDTOFactory {
		...
		private static RegistreRDTO create(
				String codi,
			    String dataRegistre,
			    PersonesRDTO persona
		) {
	...
	private static class DocumentsExpedientsRDTOFactory {
		...
		private static DocumentsExpedientsRDTO create(
				BigDecimal id,
			    String codi,
			    String nom
		) {
	...
	private static class DadesAtributsExpedientsRDTOFactory {
		...
		private static DadesAtributsExpedientsRDTO create(
				BigDecimal id,
			    String codi,
			    String valor
		) {
