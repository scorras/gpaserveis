...
public class ServeisServiceImpl implements ServeisService {
	...
	public DocsEntradaRDTO guardarDocumentEntradaEscanejatGestorDocumental(
		...
		return documentsService.guardarDocumentEntradaEscanejatGestorDocumental(guardarDocumentEntradaFitxerBDTO);
	}
	...
	public void guardarDocumentSollicitudSignat(BigDecimal documentacioIdext, BigDecimal signaturaSolicitud)
	        throws GPAServeisServiceException {
		documentsService.guardarDocumentSollicitudSignat(documentacioIdext, signaturaSolicitud);
	}
	...
	@Override
	public Persones obtenirPersonaExpedientByDocumentIdentitat(DadesExpedientBDTO dadesExpedientBDTO, String documentUsuari)
	        throws GPAServeisServiceException {
		Persones persones = null;
		List<PersonesSollicitudRDTO> listTotesPersones = new ArrayList<PersonesSollicitudRDTO>();
		if (dadesExpedientBDTO.getPersonesImplicades() != null) {
			listTotesPersones.addAll(dadesExpedientBDTO.getPersonesImplicades());
		}
		if (dadesExpedientBDTO.getPersonesInteressades() != null) {
			listTotesPersones.addAll(dadesExpedientBDTO.getPersonesInteressades());
		}
		if (listTotesPersones.size() > 0) {
			for (PersonesSollicitudRDTO persona : listTotesPersones) {
				if (StringUtils.equals(persona.getPersones().getDocumentsIdentitat().getNumeroDocument(), documentUsuari)) {
		...
		if (persones == null) {
			if (StringUtils.equals(dadesExpedientBDTO.getSollicitant().getDocumentsIdentitat().getNumeroDocument(), documentUsuari)) {
				persones = dadesExpedientBDTO.getSollicitant();
			} else if (dadesExpedientBDTO.getRepresentant() != null && StringUtils
			        .equals(dadesExpedientBDTO.getRepresentant().getDocumentsIdentitat().getNumeroDocument(), documentUsuari)) {
				persones = dadesExpedientBDTO.getRepresentant();
			}
		}
		return persones;
	}
	...
	public void registrarAuditServeisTramitadors(AuditServeisBDTO auditServeisBDTO) throws GPAServeisServiceException {
		...
		try {
			...
			log.error("Error en el registre de l'auditoria", e);
